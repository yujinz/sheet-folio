...
```
Exports the database from the running container, builds the export Docker image, and outputs a flat data bundle to `export-data/` along with a `sheet-folio-export.zip`. The zip is the same format the Web UI uses — upload it via **Settings → Import** (Merge or Replace) to restore this data in another instance.

Output format is documented in [SCHEMA.md](SCHEMA.md).

Logs milestones to `$HOME/logs/sheet-folio-export-data.log`. On failure, the last 20 lines of output are appended to the log.

## Data Backup

Saves SHA-deduplicated backups of docker app volumes and exported data, with optional upload to object storage. Volume archives are compressed `.tar.gz`; export archives are `.zip` (produced by the export Docker image). Identical data reuses the same archive. Keeps the last 5 unique-SHA backups both locally and on cloud.
