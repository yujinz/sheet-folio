 ...
 *
 * 🔄 DEMO SYNC: Each function here has a counterpart in src/lib/data.ts on the
 *    `main` branch. Both implement the DataLayer interface from src/lib/data-layer.ts.
 *    If you add a method to that interface, implement it here and in data.ts.
 *
 * Uses Dexie.js (IndexedDB) instead of sessionStorage for:
 *  - Much larger storage quota (no 5-10MB limit with base64 image data URLs)
 *  - Persistence across tab/browser close
 *  - Structured queries instead of loading the entire JSON blob on every operation
 */

import type { CategoryEntry, ImageKind, Song, SongImage, Tag, VideoLink } from "@/lib/types";
import { demoDb } from "@/lib/demo-db";
import type { DataLayer } from "@/lib/data-layer";
