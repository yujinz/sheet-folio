...
export default function TagPicker({ category, label, tags, selected, onChange, onCreate, onDelete, onUpdate, editingTags, selectedOnly, compact, defaultColor, onDefaultColorChange, onRenameCategory, onDeleteCategory }: Props) {
  ...
  const nameAltInputRef = useRef<HTMLInputElement>(null);
  const [activeInput, setActiveInput] = useState<"name" | "nameAlt">("name");
  ...
  async function handleEditSave() {
    ...
    const updated: Tag = { ...original, name: newName, nameAlt: trimmedEn, color: editColor };
  ...
  async function handleCreateFromDialog() {
    ...
    const trimmedEn = createNameAlt.trim();
    if (!trimmed && !trimmedEn) return;
    try {
      const created = await onCreate({ name: trimmed || trimmedEn, nameAlt: trimmedEn, color: createColor, category });
  ...
  async function createTag() {
    ...
    const trimmedEn = nameAlt.trim();
    if (!trimmed && !trimmedEn) return;
    try {
      const tag = await onCreate({ name: trimmed || trimmedEn, nameAlt: trimmedEn, color, category });
            ...
            <div className="mx-4 w-full max-w-xs rounded-lg bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="grid gap-2">
                ...
                <input
                  ...
                  value={createNameAlt}
                  onChange={(e) => setCreateNameAlt(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter") handleCreateFromDialog(); if (e.key === "Escape") setShowCreateDialog(false); }}
                />
                  ...
                  {category === "pitch" && (
                    <>
                      ...
                      <button
                        ...
                        onClick={() => {
                          const pitchName = locale === "en-US" ? (createNameAlt || createName) : (createName || createNameAlt);
          ...
          <div className="flex flex-wrap items-center gap-1">
            {category === "pitch" && (
              <div className="flex gap-1">
                {["♭", "♯", "♮"].map((mark) => (
                  <button key={mark} className="pill-add-button" type="button" onClick={() => {
                    const setter = activeInput === "nameAlt" ? setNameAlt : setName;
                    setter((value) => `${mark}${value}`);
                    if (activeInput === "nameAlt") {
                      nameAltInputRef.current?.focus();
                    } else {
            ...
            <input
              ...
              style={{ width: "5rem", fontSize: "12px", border: "1px solid var(--line)", borderRadius: "999px", background: "#fff", color: "var(--foreground)", padding: "3px 8px", minHeight: "24px", outline: "none" }}
              value={nameAlt}
              onChange={(event) => setNameAlt(event.target.value)}
              onFocus={() => setActiveInput("nameAlt")}
              placeholder="New Tag"
            />
            {category === "pitch" ? (
              <button
                ...
                onClick={() => {
                  ...
                  const pitchName = locale === "en-US" ? (nameAlt || name) : (name || nameAlt);
                  const c = pitchColorFromName(pitchName);
                  if (c) setColor(c);
                }}
              >
          ...
          <div className="mx-4 w-full max-w-xs rounded-lg bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid gap-2">
              ...
              <input
                ...
                placeholder={categoryLabel + " (English)"}
                value={editNameAlt}
                onChange={(e) => setEditNameAlt(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") handleEditSave(); if (e.key === "Escape") setEditTag(null); }}
              />
              <div className="flex items-center gap-2">
                {category === "pitch" && (
                  <>
                    {["♭", "♯", "♮"].map((mark) => (
                      <button key={mark} className="pill-add-button" type="button" onClick={() => {
                        ...
                        if (editNameAlt && !editName) {
                          setEditNameAlt(mark + editNameAlt);
                        } else {
                    ...
                    <button
                      ...
                      title="Assign color based on pitch octave"
                      onClick={() => {
                        const pitchName = locale === "en-US" ? (editNameAlt || editName) : (editName || editNameAlt);
                        const c = pitchColorFromName(pitchName);
                        if (c) setEditColor(c);
                      }}
                    >
